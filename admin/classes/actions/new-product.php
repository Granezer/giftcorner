...
try {
    ...
    $reg_activity->setReportType("Added Product");
    ...
    $response = $employee->newProduct($name, $price, $description, $short_desc, $product_off, $discount_start_date, $discount_end_date, $weight, $breadth, $length, $depth, $user_ses_id);

    // $images = explode('|', $response['image_urls']);
    // $target_dir = 'upload/';
    // foreach ($images as $image) 
    //     if ($image) unlink($target_dir.$image);

    // Register request as successful
    $reg_activity->setReportTypeId($response['product_id']);

    $description = $employee_Details->first_name . " added new product of ID ".$response['product_id'];
