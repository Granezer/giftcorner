...
try {
    $status = isset($_GET['status']) 
        ? $reg_activity->formatInput($_GET['status']) : 0;
    $id = isset($_GET['id']) 
    ...
    $reg_activity->setReportType("Retrieved Orders");
    ...
    $response = array();
    if ($id) $response = $getOrders->getOrder($id);
    else $response = $getOrders->getOrders($pagination, $sort, $query, $_REQUEST, $status);
    
    $description = $employee_Details->first_name . " retrieved orders";
