...
try {
    ...
    $reg_activity->setReportType("Updated Department");
    ...
    if (empty($name)) throw new Exception("Enter department name");
    ...
    $edit_department = new Settings\Department();
    $response = $edit_department->editDepartment($id, $name);

    // Register request as successful
    $reg_activity->setReportTypeId(0);

    $description = $employee_Details->first_name . " updated department ".strtoupper($name);
