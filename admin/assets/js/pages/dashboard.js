...
var KTDashboard = function() {
    var orderStatsValues = [0,0,0,0,0,0,0];
    var userPaymentStatsValues = [0,0,0];
    var productStatsValues = [0,0];
    ...
    var handleGetProductStats = function() {
        $.ajax({
            type: "GET",
            url: endPoint + "get-product-stats.php",
            success: function(response, status, xhr, $form) {
                // console.log(response['data']);
                if (response['success']) {
                    response = response['data'];
                    productStatsValues[0] = response.paused;
                    productStatsValues[1] = response.live_on_site;
                } else {
    ...
    var allProducts = function() {
        var chartContainer = KTUtil.getByID('kt_chart_products');
    ...
    var handleGetOrderStats = function() {
        $.ajax({
            type: "GET",
            url: endPoint + "get-order-stats.php",
            success: function(response, status, xhr, $form) {
                // console.log(response['data']);
                if (response['success']) {
                    response = response['data'];
                    orderStatsValues[0] = response.awaiting_payment;
                    orderStatsValues[1] = response.pending;
                    orderStatsValues[2] = response.delivered;
                    orderStatsValues[3] = response.cancelled;
                    orderStatsValues[4] = response.accepted;
                    orderStatsValues[5] = response.pending_pickup;
                    orderStatsValues[6] = response.pending_delivery;
                } else {
                    console.log(response['message']);
                }
            },
    ...
    var orderStats = function() {        
        if (!KTUtil.getByID('kt_chart_orders')) {
            return;
        }
        ...
        setTimeout(function() {
            $("#highest_order").html(Math.max(...orderStatsValues));
            $("#awaiting_payment").html(orderStatsValues[0] + '% Awaiting Payment');
            $("#pending").html(orderStatsValues[1] + '% Pending');
            $("#delivered").html(orderStatsValues[2] + '% Delivered');
            $("#cancelled").html(orderStatsValues[3] + '% Cancelled');
            $("#accepted").html(orderStatsValues[4] + '% Confirmed');
            $("#pending_pickup").html(orderStatsValues[5] + '% Pending Pickup');
            $("#pending_delivery").html(orderStatsValues[6] + '% Pendin Delivery');

            var config = {
                type: 'doughnut',
                data: {
                    datasets: [{
                        data: orderStatsValues,
                        backgroundColor: [
            ...
            var ctx = KTUtil.getByID('kt_chart_orders').getContext('2d');
            var myDoughnut = new Chart(ctx, config);
        }, 500);
    }

    var handleGetUserPaymentStats = function() {
        $.ajax({
            type: "GET",
            url: endPoint + "get-payment-stats.php",
            success: function(response, status, xhr, $form) {
                // console.log(response['data']);
                if (response['success']) {
                    response = response['data'];
                    userPaymentStatsValues[0] = response.pending;
                    userPaymentStatsValues[1] = response.confirmed;
                    userPaymentStatsValues[2] = response.cancelled;
                } else {
                    console.log(response['message']);
                }
            },
    ...
    var userPayments = function() {
        if ($('#kt_chart_user_payments').length == 0) {
            return;
        }

        setTimeout(function() {
            $("#payment_pending").html(userPaymentStatsValues[0] + '% Pending');
            $("#payment_confirmed").html(userPaymentStatsValues[1] + '% Confirmed');
            $("#payment_cancelled").html(userPaymentStatsValues[2] + '% Cancelled');

            Morris.Donut({
                element: 'kt_chart_user_payments',
                data: [{
                        label: "Pending Payments",
                        value: userPaymentStatsValues[0]
                    },
                    {
                        label: "Confirmed Payments",
                        value: userPaymentStatsValues[1]
                    },
                    {
                        label: "Cancelled Payments",
                        value: userPaymentStatsValues[2]
                    },
                ],
                colors: [
                    KTApp.getStateColor('warning'),
